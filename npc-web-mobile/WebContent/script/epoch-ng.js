...
!!*/
/**
 * VasWorks Calendar control extension
 * Matija Obreza, September 2009 for VasWorks
 */
if (!VasWorks) var VasWorks = {};
VasWorks.DatePicker = Class.create();
/**
 * Show a datepicker
 * @param input input field DOM control
 * @param dateFormat Date format
 */
VasWorks.DatePicker.show = function(input, dateFormat, defaultValue) {
	input.setAttribute('autocomplete', 'off');
	var date=input.dateObj;
	if (!date) input.dateObj=defaultValue;
	VasWorks.DatePicker.showEpoch(input, dateFormat);
}

VasWorks.DatePicker.showEpoch = function(input, dateFormat) {
	if (!VasWorks.DatePicker.epochControl) {
		VasWorks.DatePicker.epochControl = new Epoch('single-epoch', 'popup', null, { format: 'dd/MM/yyyy', multiselect: false });
	}
	VasWorks.DatePicker.epochControl.setTarget(input, true, dateFormat);
}
/**
* The main Epoch class.  All publicly-accessible methods and properties are called from this class
*/
function Epoch(name,mode,targetelement,options) {
	...
	*/
	function createCalendar() {
		...
		Element.addClassName(self.calendar,'calendar');
		self.calendar.style.display = 'none'; //default to invisible
		//to prevent IE from selecting text when clicking on the calendar
		Event.observe(self.calendar,'selectstart', function() {return false;});
		Event.observe(self.calendar,'drag', function() {return false;});
		...
		Element.addClassName(self.calendar.celltable,'cells');
		...
		Event.observe(self.calendar,'mouseover',cal_onmouseover);
		Event.observe(self.calendar,'mouseout',cal_onmouseout);
	}
	//-----------------------------------------------------------------------------
	/**
	...
	*/
	function createMainHeading() {
		...
		Element.addClassName(container,'mainheading');
		...
		Event.observe(monthUp,'click',function(){self.nextMonth();});
		Event.observe(monthDn,'click',function(){self.prevMonth();});
		Event.observe(self.monthSelect,'change',selectonchange);
		Event.observe(self.yearSelect,'change',selectonchange);
	...
	*/
	function createFooter() {
		...
		Event.observe(clearSelected,'click',function() {self.resetSelections(false);});
		...
		if(self.mode == 'popup') {
			...
			Event.observe(closeBtn,'click',function(){self.hide();});
			Element.addClassName(closeBtn,'closeBtn');
	...
	*/
	function createDayHeading() {
		...
		Element.addClassName(self.calHeading,'caldayheading');
		...
		if(self.showWeeks) {
			...
			Element.addClassName(th,'wkhead');
			tr.appendChild(th);
		}
			...
			if(self.showWeeks) { //if we are showing the week headings
				if(i % 8 == 0) {
					...
					if(self.selectMultiple) { //if selectMultiple is enabled, create the associated weekObj objects
						...
						Element.addClassName(td,'wkhead');
					}
	...
	* @param bool focus
	*/
	self.setTarget = function (targetelement, focus, dateFormat)
	{
		if (dateFormat) self.options.format=dateFormat;
		...
		if(self.mode == 'popup') {
			...
			if(self.tgt) {
				Event.stopObserving(self.tgt,'focus',popupFocus);
				Event.stopObserving(self.tgt,'blur',popupBlur);
				Event.stopObserving(self.tgt,'keydown',popupKeyDown);
			}
			...
			Event.observe(self.tgt,'focus',popupFocus);
			Event.observe(self.tgt,'blur',popupBlur);
			Event.observe(self.tgt,'keydown',popupKeyDown);
	...
	*/
	...
	self.mode = mode;
	self.selectMultiple = (self.options.multiselect == true); //'false' if not true or not set at all
	...
	if (self.options.value) {
		var selectedDate=self.options.value;
		selectedDate.canSelect=true;
		self.selectDates([selectedDate],true,false,true);
		self.goToMonth(self.options.value.getYear()+1900, self.options.value.getMonth()+1);
	}
}
//-----------------------------------------------------------------------------
/*****************************************************************************/
...
function CalHeading(owner,tableCell,dayOfWeek) {
	...
	Event.observe(tableCell,'mouseup',DayHeadingonclick);
}
/*****************************************************************************/
/**
...
function WeekHeading(owner,tableCell,week,tableRow) {
	...
	Event.observe(tableCell,'mouseup',weekHeadingonclick);
}
/*****************************************************************************/
/**
...
function CalCell(owner,tableCell,dateObj,row,week) {
		...
		if(self.date.canSelect) {
			if(owner.selectMultiple == true) { //if we can select multiple cells simultaneously, add the currently selected self's date to the dates array
				...
				if(owner.mode == 'popup') { //update the target element's value and hide the calendar if in popup mode
					owner.tgt.value = self.date.dateFormat(owner.options.format); //use the default date format defined in dateFormat
	...
	self.setURL = function(href,type) {
		if(href) {
			if(type == 'js') { //Make the WHOLE cell be a clickable link
				Event.observe(self.tableCell,'mousedown',function(){window.location.href = href;});
			}
	...
	*/
	...
	Event.observe(tableCell,'click', calCellonclick);
	Event.observe(tableCell,'mouseover', calCellonmouseover);
	Event.observe(tableCell,'mouseout', calCellonmouseout);
	self.setClass();
}
/*****************************************************************************/
...
Date.prototype.dateFormat = function(format)
{
	...
	while (i_format < format.length) {
		...
		if (value[token]==null && token2!=null) { token=token2; i_format--; }
